...
Problem:
    ...
    following way:
        - The first element of the array occupies the first 8
          bits of M;
        - The second element occupies next 8 bits, and so on.
        - Return the obtained integer M.

    Note: the phrase "first bits of M" refers to the least significant
    bits of M - the right-most bits of an integer. For further
...
def arrayPacking(a: list[int]) -> int:
    x = ''.join(['0' * (8 - (len(bin(a[i])[2:]))) + bin(a[i])[2:] for i in range(len(a)-1, -1, -1)])
